...
function Courses() {
  ...
  const handleEnroll = () => {
    ...
    let userCourses = allCourses[user.id] || [];

    const alreadyEnrolled = userCourses.find((c) => c.id === course.id);
    if (alreadyEnrolled) {
    ...
    allCourses[user.id] = userCourses;
  ...
  const handlePlay = (lessonId) => {
    ...
    const userCourses = allCourses[user.id] || [];
    const enrolled = userCourses.find((c) => c.id === course.id);

    if (!enrolled) {
      toast.error("Please enroll in the course first");
      return;
    }
    navigate(`/play/${course.id}/${lessonId}`);
    ...
    if (!courseProgress.includes(lessonId)) {
      ...
      userProgress[course.id] = courseProgress;
      allProgress[user.id] = userProgress;
      localStorage.setItem("courseProgress", JSON.stringify(allProgress));
    }
  };

  if (!course) return <h2 style={{ color: "white", marginTop: "50px", textAlign: "center" }}>Loading...</h2>;
  const user = JSON.parse(localStorage.getItem("user"));
  const allProgress = JSON.parse(localStorage.getItem("courseProgress")) || {};
    ...
    <>
      ...
      <div className="course-card">
        ...
        <div className="details">
          ...
          <button onClick={handleEnroll} className="btn btn-primary" style={{ backgroundColor: "#e85d26", color: "white", fontWeight: "600", border: "#e85d26", marginBottom: "10px" }} >
            Enroll
          </button>
          {progressPercentage > 0 && (
            <div className="progress" style={{ height: "12px", marginBottom: "10px" }}>
              <div
                ...
                role="progressbar"
                style={{ width: `${progressPercentage}%`, backgroundColor: "#e85d26" }}
          ...
          {progressPercentage > 0 && <small style={{ color: "white" }}>{progressPercentage}% completed</small>}
        </div>
      </div>
          ...
          <tbody>
            {course.lessons.map((lesson) => (
              <tr key={lesson.lessonId}>
                ...
                <td>
                  <button onClick={() => handlePlay(lesson.lessonId)}  className="btn btn-success" style={{ backgroundColor: "#e85d26", color: "white", fontWeight: "600", border: "#e85d26" }} >
                    Play
                  </button>
                </td>
              </tr>
